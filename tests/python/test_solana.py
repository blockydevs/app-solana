...
class TestComputeBudgetTransfer:
    ...
    def test_solana_transfer_with_compute_budget_limit(self, backend, navigator, test_name):
        sol = SolanaClient(backend)
        from_public_key = sol.get_public_key(SOL_PACKED_DERIVATION_PATH)
        ...
        message: bytes = Message(from_public_key, [ instruction_unit_price, instruction_unit_limit, instruction]).serialize()
